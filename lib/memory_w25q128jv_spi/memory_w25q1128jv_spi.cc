...
int MemoryW25q1128jvSpi::Init() {
    ...
    do {
        ...
        //if memory is full, return error
        if(address == 0xFFFFFF) return -2;

    //end if data value struct hasn't been written in block
    } while (buffer[0] != 0xFF);
...
MemoryW25q1128jvSpi::State MemoryW25q1128jvSpi::ChipWrite(uint8_t (&data)[64]) {
    ...
    // if address variable goes over the amount of pages memory has, 0xFFFF00 is last 64 bye block
    // no more collected data will be written into the memory
    if (address > 0xFFFF00) _state = State::ERROR;
...
int MemoryW25q1128jvSpi::ChipReadDump(uint8_t (&chipData)[64]) {
    ...
    // stop reading at last 64 memory struct
    if (dumpAddress > 0xFFFFFF)
    {
        HAL_GPIO_WritePin(_csPort, _csPin, GPIO_PIN_SET);
        return -2;
    }
